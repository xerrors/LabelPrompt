...
def main():
    ...
    assert args.k * args.r == 0 and args.k + args.r > 0
    ...
    for seed in args.seed:
        ...
        # Set up dir
        n = args.k if args.k else int(args.r * 100)
        setting_dir = os.path.join(output_dir, f"{n}-{seed}")
        ...
        with open(os.path.join(setting_dir, "train.txt"), "w") as f:
            ...
            for label in label_list:
                size = args.k if args.k else int(len(label_list[label]) * args.r)
                for line in label_list[label][:size]:  # train中每一类取前k个数据
